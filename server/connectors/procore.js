/**
 * Procore Connector
 * Handles connection testing and data synchronization with Procore
...
async function testConnection(credentials, config) {
  ...
  // In production this would exchange the refresh_token via the Procore OAuth
  // endpoint and call the REST API (e.g. GET /rest/v1.0/me)
  return {
    success: true,
    message: 'Procore connection verified successfully',
    details: { company_id: config.company_id, api_version: 'v1.0' }
  };
}
...
async function sync(integration, db) {
  ...
  const customers = await db.query('SELECT id, name FROM customers WHERE 1=1');
  const dispatches = await db.query('SELECT id, title, status FROM dispatches WHERE 1=1');
  ...
  return {
    ...
    records_failed: 0,
    details: { vendors_synced: customers.length, projects_synced: dispatches.length }
  };
}
