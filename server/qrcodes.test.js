...
describe('POST /api/qrcodes/generate', () => {
  it('should create a QR code', async () => {
    ...
    const res = await request(app).post('/api/qrcodes/generate').send({ customer_id: 'cust-1', location_name: 'Main Office' });
  ...
  it('should return 500 on db error', async () => {
    ...
    const app = createTestApp();
    const res = await request(app).post('/api/qrcodes/generate').send({ customer_id: 'cust-1', location_name: 'Office' });
...
describe('POST /api/qrcodes/validate', () => {
  it('should validate an active QR code', async () => {
    ...
    mockDb.get.mockResolvedValue(qrCode);
    const app = createTestApp();
    const res = await request(app).post('/api/qrcodes/validate').send({ qr_code_data: 'FIELDFORGE-qr-1' });
    expect(res.status).toBe(200);
    expect(res.body).toEqual(qrCode);
  });
  ...
  it('should return 404 for invalid or inactive QR code', async () => {
    ...
    const app = createTestApp();
    const res = await request(app).post('/api/qrcodes/validate').send({ qr_code_data: 'INVALID' });
...
describe('POST /api/qrcodes/checkin', () => {
  ...
  it('should check in a technician', async () => {
    ...
    mockDb.run.mockResolvedValue({});
    const app = createTestApp();
    const res = await request(app).post('/api/qrcodes/checkin').send(checkinPayload);
    expect(res.status).toBe(200);
  ...
  it('should emit socket event on successful check-in', async () => {
    ...
    mockDb.run.mockResolvedValue({});
    const app = createTestApp();
    await request(app).post('/api/qrcodes/checkin').send(checkinPayload);
...
describe('POST /api/qrcodes/checkout/:checkInId', () => {
  it('should check out a technician', async () => {
    ...
    const app = createTestApp();
    const res = await request(app).post('/api/qrcodes/checkout/ci-1').send({ notes: 'Work complete' });
    expect(res.status).toBe(200);
    expect(res.body).toEqual(checkIn);
  ...
  it('should emit socket event on successful check-out', async () => {
    ...
    mockDb.get.mockResolvedValue(checkIn);
    const app = createTestApp();
    await request(app).post('/api/qrcodes/checkout/ci-1').send({ notes: 'Done' });
    const mockIo = app.get('io');
  ...
  it('should return 500 on db error', async () => {
    mockDb.run.mockRejectedValue(new Error('DB error'));
    const app = createTestApp();
    const res = await request(app).post('/api/qrcodes/checkout/ci-1').send({ notes: 'Done' });
    expect(res.status).toBe(500);
